...
// DELETE - Delete developer with cascading deletion
export async function DELETE(request: NextRequest) {
  try {
    ...
    const result = await MySQLDatabase.deleteDeveloperWithCascade(parseInt(id));
    
    if (!result.success) {
      return NextResponse.json({
        success: false,
        message: 'Developer not found or could not be deleted'
      }, { status: 404 });
    }

    const message = result.deletedProjects > 0 
      ? `Developer deleted successfully along with ${result.deletedProjects} associated project(s)`
    ...
    return NextResponse.json({
      success: false,
      message: 'Failed to delete developer. This may be due to database constraints.'
    }, { status: 500 });
  }
}