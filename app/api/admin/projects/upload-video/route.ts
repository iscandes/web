...
export async function POST(request: NextRequest) {
  try {
    ...
    // Enhanced file size validation (increased for videos)
    const maxSize = 250 * 1024 * 1024; // 250MB for videos and animation sequences
    ...
    const existingProject = await MySQLDatabase.executeQuery(
      'SELECT media_files, featured_video FROM projects WHERE id = ?',
      [projectId]
    );
    ...
    const newMediaEntry = {
      ...
      name: file.name,
      url: publicUrl,
      file_path: publicUrl, // Add file_path for compatibility
    ...
    // Update the projects table with media_files
    await MySQLDatabase.executeQuery(
      'UPDATE projects SET media_files = ? WHERE id = ?',
      [JSON.stringify(mediaFiles), projectId]
    );
    
    // If this is a video, update the featured_video field
    // Always update for videos to ensure consistency
    if (detectedMediaType === 'video') {
      console.log('Updating featured_video for project:', projectId, 'with URL:', publicUrl);
      await MySQLDatabase.executeQuery(
