...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  const handleProjectClick = (e: React.MouseEvent) => {
    // Prevent default only if it's not coming from a Link component's click
    if (!(e.target as HTMLElement).closest('a')) {
      e.preventDefault();
      const destination = project.slug || project.id;
      console.log('Project card clicked, navigating to:', destination);
      router.push(`/projects/${destination}`, { scroll: true });
    }
  };
