  ...
  const createNewDeveloper = async (name: string) => {
    try {
      ...
      if (response.ok) {
        ...
        alert('New developer created successfully!');
      } else {
      ...
      alert('Error creating new developer');
    }
  };
    ...
    try {
      ...
      if (editingProject?.id) {
        ...
        if (response.ok && result.success) {
          setFormData(prev => ({ ...prev, featured_video: result.data.filePath }));
            ...
            try {
              ...
              if (videoResponse.ok && videoResult.success) {
                ...
                if (updateResponse.ok) {
                  ...
                  alert('Project and video uploaded successfully!');
                } else {
                  console.error('Failed to update project with video URL');
                  alert('Video uploaded but failed to update project. Please refresh the page.');
                }
              } else {
                console.error('Video upload failed:', videoResult);
                alert(`Project created but video upload failed: ${videoResult.error || 'Unknown error'}`);
              }
              ...
              alert('Project created but video upload failed. Please try uploading the video again by editing the project.');
              // Clean up the pending video file even on error
              setPendingVideoFile(null);
            }
          } else {
            console.error('No project ID found in response:', result);
            alert('Project created but could not upload video - no project ID found. Please edit the project to add the video.');
            // Clean up the pending video file
            setPendingVideoFile(null);
          }
        }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">1 Bathroom</label>
                    <input
                      type="number"
                      min="0"
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">2 Bathroom</label>
                    <input
                      type="number"
                      min="0"
