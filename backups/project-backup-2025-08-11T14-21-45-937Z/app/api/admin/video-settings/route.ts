...
export async function POST(request: NextRequest) {
  try {
    ...
    const { videoType, videoUrl, youtubeUrl, title, description, isActive }: VideoSettings = body;
    ...
    if (dbAvailable) {
      ...
      const result = await MySQLDatabase.executeQuery(
        `INSERT INTO video_settings 
         (video_type, video_url, youtube_url, title, description, is_active) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        [videoType, videoUrl || null, youtubeUrl || null, title, description || null, isActive ? 1 : 0]
      );
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, videoType, videoUrl, youtubeUrl, title, description, isActive }: VideoSettings = body;
    ...
    if (dbAvailable) {
      ...
      await MySQLDatabase.executeQuery(
        `UPDATE video_settings 
         SET video_type = ?, video_url = ?, youtube_url = ?, title = ?, description = ?, is_active = ?
         WHERE id = ?`,
        [videoType, videoUrl || null, youtubeUrl || null, title, description || null, isActive ? 1 : 0, id]
      );
