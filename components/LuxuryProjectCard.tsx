...
export default function LuxuryProjectCard({ project, index }: LuxuryProjectCardProps) {
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    if (!(e.target as HTMLElement).closest('a') && !(e.target as HTMLElement).closest('button')) {
      e.preventDefault();
      // Use Link-compatible navigation to avoid RSC conflicts
      router.push(`/projects/${project.slug || project.id}`, { scroll: true });
    }
  };
  ...
  return (
    <div
      ...
      className="group relative performance-optimized cursor-pointer"
    >
